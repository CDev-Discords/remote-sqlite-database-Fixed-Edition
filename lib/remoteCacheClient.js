...
class RemoteCacheClient extends EventEmitter {
    ...
    /**
     * Filter the Map by searching for a certain value
     * @param {string} filter | function for filtering the data
     * @returns {map} with the filtered data
     */
    async filter(filter = null) {
        if (filter && lodash.isFunction(filter)) {
            return new Map([...this.cache].filter(([key, value]) => filter(value)));
        } else {
     ...
     * It is better, because it can support FILTERING!
     * for e.g, keyArray(d => d.settings) will return an Array of all keys with the settings-key
     * @returns {array} of all keys
     */
    async keyArray(filter = null) {
        if (filter && lodash.isFunction(filter)) {
            let keys = [];
            for (const [key, value] of this.cache) {
                if (filter(value)) keys.push(key);
            }
            return keys;
        } else {
